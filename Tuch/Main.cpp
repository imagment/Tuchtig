...
#define NAMEVERSION "Tuchtig 1.0.0 alpha"
...
unordered_map<string, boost::any> globalArrays;
...
bool IsTCFunction(const string& funcName)
{
	return startsWith(funcName, "TC.");
}
...
boost::any EvalExpression(const string& ex, unordered_map<string, boost::any>& variableValues)
{
	...
	bool isTC = split(expression, '.')[0] == "TC";
	// If no operations are applied, then return self
	if ((countOutsideParenthesis(expression, '+') == 0 && countOutsideParenthesis(expression, '-') == 0 && countOutsideParenthesis(expression, '*') == 0 && countOutsideParenthesis(expression, '/') == 0 && countOutsideParenthesis(expression, '^') == 0) || split(expression, '.')[0] == "TC")
	{
		//bool isFunc = IsFunction(split(expression, '(')[0]);
		if (isFunc && !inQuotes)
		{
...
#endif
			vector<boost::any> funcArgs = VarValues(argList, variableValues);
			return TCFunction(split(expression, '(')[0], funcArgs);
		}
		else
	...
	for (int i = 0; i < expression.size(); i++)
	{
		...
		if (isalpha(expression[i]) || expression[i] == '_')
		{
			...
			else if (split(name, '.')[0] == "TC" && !inQuotes)
			{
				// start -> FuncCall(0, x, OtherFunc(a))
				// changeto -> 0, x, OtherFunc(a)
				string insideFunArgs = betweenChars(expression, '(', ')');
...
#endif
				vector<boost::any> funcArgs = VarValues(argList, variableValues);
				string returnVal = AnyAsString(TCFunction(split(expression, '(')[0], funcArgs));
				newExpression += returnVal;
			}
			else
			{
...
boost::any ProcessLine(const vector<vector<string>>& words, int& lineNum, unordered_map<string, boost::any>& variableValues)
{
	...
	// If print statement (deprecated, now use TC.System.Print() function)
	...
	// Check if it is TC Builtin function call
	else if (startsWith(words.at(lineNum).at(0), "TC."))
	...
	else if (words.at(lineNum).at(0) == "global")
	{
		...
		{
			LogCriticalError("Error at line: " + to_string(lineNum) + ", " + unWrapVec(words.at(lineNum)) + ", couldn't initialize variable. (Probably you missed a space?)");
		}
		return nullType;
	}
	...
	// If declaring rend loop, loop forever.
	
	else if (words.at(lineNum).at(0) == "rend")
	{
	...
	// If declaring a for loop, get three parts
	else if (words.at(lineNum).at(0) == "for")
	{
		vector<vector<string>> forContents;
		vector<string> declarationParameters,statement,operation;
		...
		for (int w = 1; w < (int)words.at(lineNum).size(); w++) {
			...
			else if (count(words.at(lineNum).at(w), '{') == 0 && semicolons==0)
				declarationParameters.push_back(words.at(lineNum)[w]);
			else if (count(words.at(lineNum).at(w), '{') == 0 && semicolons==1)
				statement.push_back(words.at(lineNum)[w]);
			else if (count(words.at(lineNum).at(w), '{') == 0 && semicolons==2)
				operation.push_back(words.at(lineNum)[w]);
			else{
				operation.push_back(replace(words.at(lineNum)[w], "{", ""));
				//printf("a");
			}
		}
		...
		while (lineNum < (int)words.size())
		{
			...
			if (numOfBrackets == 0)
				break;
			forContents.push_back(words.at(lineNum));
			lineNum++;
		}

		// If the statement is already false, don't bother gathering the contents
		if (BooleanLogic(statement.at(0), statement.at(1), statement.at(2), variableValues) == false) {
			lineNum++;
		...
		// Loop while true
		while (BooleanLogic(statement.at(0), statement.at(1), statement.at(2), variableValues))
		{
			//Iterate through all lines in while loop
			for (int lineNum = 0; lineNum < (int)forContents.size(); lineNum++)
			{
				if (forContents.at(lineNum).at(0) == "continue")
					break; // Stops iterating through lines and return to beginning
				if (forContents.at(lineNum).at(0) == "break")
					return nullType; // Stops iterating through lines and leave while loop
				boost::any returnVal = ProcessLine(forContents, lineNum, variableValues);
				if (!returnVal.empty()) {
					try
					{
						BREAK t = any_cast<BREAK>(returnVal);
	...
	else if (words.at(lineNum).at(0) == "loop")
	{
	    // Initialize loop contents and parameters
	    
	    vector<vector<string>> loopContents;
	    int numIterations = 1; // Default to 1 iteration if not specified

	    // Extract loop parameters
	    for (int w = 1; w < (int)words.at(lineNum).size(); w++) {
	    	
	      if (words.at(lineNum).at(w).find("{") != string::npos) {
		   
		    std::string A = words.at(lineNum).at(w);
		    size_t pos = A.find("{");
		    if (pos != std::string::npos) {
		    	A.replace(pos, 1, "");
		    }
		    //cout << A;
		    numIterations = AnyAsInt(GetVariableValue(A, variableValues));
		    //cout << AnyAsInt(GetVariableValue(A, variableValues));
		} else {
		    // Handle loop parameter without brackets
		    
		    const std::string& A=words.at(lineNum).at(w);
		    //cout << A;
		     numIterations = AnyAsInt(GetVariableValue(A, variableValues));
		     //cout << AnyAsInt(GetVariableValue(A, variableValues));
		    break; // GetVariableValue("EXIT_WHEN_DONE", globalVariableValues)
		}
	    }

	    // Gather the loop's content
	    lineNum++;
	    int numOfBrackets = 0; // Track brackets for nested content
	    while (lineNum < (int)words.size())
	    {
		numOfBrackets += countInVector(words.at(lineNum), "{") - countInVector(words.at(lineNum), "}");
		if (numOfBrackets == 0 && words.at(lineNum).at(0).find("}") != string::npos)
		    break;
		loopContents.push_back(words.at(lineNum));
		lineNum++;
	    }
	    ...
	    {
		for (int i = 0; i < (int)loopContents.size(); i++)
		{
		    if (loopContents.at(i).at(0) == "continue")
		        break; // Stops iterating through lines and return to beginning of loop
		    if (loopContents.at(i).at(0) == "break")
		        return nullType; // Stops iterating through lines and leaves the loop
		    boost::any returnVal = ProcessLine(loopContents, i, variableValues);
		    if (!returnVal.empty()) {
		        try
		        {
		            BREAK t = any_cast<BREAK>(returnVal);
	...
	// If declaring a dowhile loop, loop until false
	
	else if (words.at(lineNum).at(0) == "dowhile")
	{
		vector<vector<string>> whileContents;
		vector<string> whileParameters;
		...
		do {
			...
			isFirst=0;
		} while (BooleanLogic(whileParameters.at(0), whileParameters.at(1), whileParameters.at(2), variableValues) || isFirst);
		return nullType;
	}
...
boost::any ExecuteFunction(const string& functionName, const vector<boost::any>& inputVarVals)
{
	...
	unordered_map<string, boost::any> Arrays = {};
...
std::vector<std::string> splitScript(const std::string& script) {
    ...
    for (size_t i = 0; i < script.size(); ++i) {
        ...
        char nextChar = (i + 1 < script.size()) ? script[i + 1] : '\0';
        
        

        // Check for start/end of quotes
        if (currentChar == '"') {
            inQuotes = !inQuotes;
        }
        ...
        // If not in quotes or comment, split lines on ';' or '\n'
        if (!inQuotes && !inComment) {
            if (currentChar == ';' || currentChar == '\n') {
                // Push current line to vector if it's not empty
        ...
        // Append character to current line if not a comment
        if (!inComment) {
...
int parseTuchtig(string script)
{
	
	
	vector<string> beforeProcessLines = splitScript(script);
...
#endif
#endif
				parseTuchtig(scriptTextContents);
			}
...
int main(int argc, char* argv[])
{
	...
	if (argc > 1 || EXAMPLE_PROJECT)
	{
		...
		if (EXAMPLE_PROJECT)
			scriptPath = "D:\\Code\\Z-Sharp\\examples\\Platformer\\script.TC";
		else
			scriptPath = argv[1];
#if DEVELOPER_MESSAGES
	...
	{ // If no script is provided as an argument throw error
		LogWarning("No script provided! Please drag and drop .TC file over interpreter executable file, or provide it's path as a command-line argument.");
